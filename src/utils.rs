...
pub fn cosine_similarity(vec1: &[f32], vec2: &[f32], normalized: bool) -> f32 {
    let dot_product: f32 = vec1.par_iter().zip(vec2.par_iter()).map(|(a, b)| a * b).sum();
...
pub fn pearson_correlation(vec1: &[f32], vec2: &[f32]) -> f32 {
    ...
    let numerator: f32 = vec1.par_iter().zip(vec2.par_iter())
        .map(|(a, b)| (a - mean1) * (b - mean2))
        .sum();

    let denom1: f32 = vec1.par_iter()
        .map(|a| (a - mean1).powi(2))
        .sum::<f32>()
        .sqrt();

    let denom2: f32 = vec2.par_iter()
