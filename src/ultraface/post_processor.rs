...
impl UltraOutput {
    pub fn new(outputs: Vec<Value>) -> Result<UltraOutput, OrtError> {
        let scores: OrtOwnedTensor<f32, _> = outputs[0].try_extract()?;
        let confidences_view = scores.view();
        let confidences = confidences_view.slice(s![0, .., 1]);

        let bboxes: OrtOwnedTensor<f32, _> = outputs[1].try_extract()?;
        let bbox_view = bboxes.view();
